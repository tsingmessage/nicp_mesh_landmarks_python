...
betas = np.linspace(3, 0, 20)

def nonrigidIcp(sourcemesh,targetmesh, landmark_list_source = None, landmark_list_target = None, landmark_flag = 0):
    ...
    for num_,alpha_stiffness in enumerate(alphas):
        beta_iter = betas[num_]
        print('alpha, beta:',alpha_stiffness, beta_iter)
        ...
        for i in range(3):
            ...
            #print(Dl.shape, Ul.shape, A.shape, B.shape)
            if landmark_flag == 1:
                Dl = sparse.lil_matrix((len(landmark_list_source),4*n_source_verts),dtype=np.float32)
                Ul = target_vertices[landmark_list_target,:]
                for i_ls in range(len(landmark_list_source)):
                    cor = landmark_list_source[i_ls]
                    start_index = (cor+1)*4-4
                    end_index = (cor+1)*4   # + 1 is to convert matlab into python
                    #print(start_index,end_index)
                    Dl[i_ls,start_index:end_index] = [source_vertices[cor,0], source_vertices[cor,1],source_vertices[cor,2],1]
                    #print(Dl[i_ls,start_index:end_index])
                    #exit(0)
               # print('with landmark')
                #print(Dl)
                #exit(0)
